...
async function handleAdminLogin(e) {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            loadProducts(); // Load products setelah login
        } else {
...
async function loadDashboardMetrics() {
    try {
        const response = await fetch(`${BASE_URL}/api/admin/metrics.php`, {
            credentials: 'include'
        });
        ...
        if (data.status === 'success') {
            ...
            document.getElementById('total-products').textContent = data.metrics.totalProducts;
            document.getElementById('active-orders').textContent = data.metrics.activeOrders;
            document.getElementById('total-users').textContent = data.metrics.totalUsers;
            document.getElementById('total-revenue').textContent = formatCurrency(data.metrics.totalRevenue);
            
            // Update charts if any
            if (data.metrics.revenueChart) {
                updateRevenueChart(data.metrics.revenueChart);
            }
            
            if (data.metrics.orderChart) {
                updateOrderChart(data.metrics.orderChart);
            }
        } else {
...
async function saveProduct() {
    try {
        ...
        if (imageFile) {
            const imageUrl = await handleImageUpload(document.getElementById('product-image'));
            if (imageUrl) {
...
async function deleteProduct(id) {
    ...
    try {
        const response = await fetch(`${BASE_URL}/api/products.php?id=${id}`, {
            method: 'DELETE',
            credentials: 'include'
        });
...
function loadOrders() {
    ...
    orders.forEach((order, index) => {
        ...
        tr.innerHTML = `
            ...
            <td>
                <div class="fw-medium">${formatCurrency(order.payment.total)}</div>
                ...
                <div class="modal-body">
                    ...
                    <div class="row mt-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <p><strong>Subtotal:</strong> ${formatCurrency(order.payment.subtotal)}</p>
                            <p><strong>Shipping:</strong> ${formatCurrency(order.payment.shipping)}</p>
                            <p><strong>Total:</strong> ${formatCurrency(order.payment.total)}</p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <table class="table">
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td>${formatCurrency(order.payment.subtotal)}</td>
                                </tr>
                                <tr>
                                    <td colspan="4" class="text-end fw-bold">Shipping:</td>
                                    <td>${formatCurrency(order.payment.shipping)}</td>
                                </tr>
                                <tr>
                                    <td colspan="4" class="text-end fw-bold">Total:</td>
                                    <td>${formatCurrency(order.payment.total)}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                ...
                <table class="table">
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td class="text-end">${formatCurrency(order.payment.subtotal)}</td>
                        </tr>
                        <tr>
                            <td colspan="3" class="text-end fw-bold">Shipping:</td>
                            <td class="text-end">${formatCurrency(order.payment.shipping)}</td>
                        </tr>
                        <tr>
                            <td colspan="3" class="text-end fw-bold">Total:</td>
                            <td class="text-end fw-bold">${formatCurrency(order.payment.total)}</td>
                        </tr>
                    </tfoot>
                </table>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold">${formatCurrency(totalRevenue)}</p>
                </div>
            </div>
            <div class="mt-4">
                <p class="text-sm text-gray-500">
                    <span class="text-blue-600">
                        <i class="fas fa-arrow-up"></i> ${formatCurrency(recentRevenue)}
                    </span>
...
async function handleImageUpload(fileInput) {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            preview.innerHTML = `<img src="${BASE_URL}${data.url}" class="img-thumbnail" style="max-width: 200px">`;
